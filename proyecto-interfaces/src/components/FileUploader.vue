		...
		<form @submit.prevent="handleSubmit">
				...
				<div class="control">
					<div class="select">
						<select v-model="fileType" id="fileType" required @change="clearPreviews">
							...
							<option value="vtt">Subtítulos</option>
						</select>
					</div>
				</div>
			</div>
			...
			<div v-if="fileType === 'audio'">
				<div v-for="index in 3" :key="index" class="field">
					<label class="label">Audio Tangram {{ index }}</label>
					<div class="control">
						<input type="file" :accept="'audio/*'" @change="handleAudioInput($event, index - 1)" />
					</div>
				</div>
			</div>
			...
			<div v-if="fileType === 'video'">
				...
				<div class="control">
					<input type="file" id="videoInput" accept="video/*" @change="handleVideoInput" />
				</div>
			</div>
			...
			<div v-if="fileType === 'pdf'">
				...
				<div class="control">
					<input type="file" id="pdfInput" accept="application/pdf" @change="handlePdfInput" />
				</div>
			</div>
			...
			<div v-if="fileType === 'vtt'">
				...
				<div class="control">
					<input type="file" id="vttInput" accept=".vtt" @change="handleVttInput" />
				</div>
			</div>
			...
			<div v-if="previews.length > 0" id="previewContainer" class="box">
				<div v-for="(preview, index) in previews" :key="index" class="preview">
					...
					<span v-if="preview.dimensions" class="image-dimensions"> ({{ preview.dimensions }}) </span>
					<span v-if="preview.format" class="video-format"> (Formato: {{ preview.format }}) </span>
